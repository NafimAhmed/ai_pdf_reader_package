...
class PdfAudioReaderState {
  ...
  bool get hasPdf => pdfBytes != null;
  bool get hasExtractedText => extractedText.trim().isNotEmpty;
  bool get hasSummary => summaryText.trim().isNotEmpty;
...
class PdfAudioReaderController extends ChangeNotifier {
  ...
  Future<void> _initialize() async {
    ...
    _tts.setProgressHandler((String text, int start, int end, String word) {
      ...
      final DateTime now = DateTime.now();
      final bool shouldRefreshTextUi = _lastTextUiAt == null ||
          now.difference(_lastTextUiAt!).inMilliseconds >= 180 ||
          globalEnd >= _state.extractedText.length;
  ...
  Future<String> generateSummary({
    int maxSentences = 5,
    int minSentenceLength = 25,
  }) async {
    if (_state.extractedText.trim().isEmpty) {
      _updateState(
        _state.copyWith(
          summaryText: '',
          statusMessage: 'No text available for summary.',
        ),
      );
      return '';
    }
    ...
    _updateState(
      _state.copyWith(
        ...
        statusMessage: 'Generating local summary...',
      ),
    );

    try {
      final String summary = LocalPdfSummarizer.summarize(
        _state.extractedText,
      ...
      _updateState(
        _state.copyWith(
          summaryText: summary,
          isGeneratingSummary: false,
          statusMessage: summary.trim().isEmpty
      ...
      _updateState(
        _state.copyWith(
          isGeneratingSummary: false,
          statusMessage: 'Summary failed: $e',
        ),
      );
      return '';
    }
  ...
  String getSummary({
    ...
    if (_state.extractedText.trim().isEmpty) return '';

    return LocalPdfSummarizer.summarize(
      _state.extractedText,
      maxSentences: maxSentences,
      minSentenceLength: minSentenceLength,
    );
...
class _PdfAudioReaderViewState extends State<PdfAudioReaderView> {
  ...
  Widget _buildTopControlPanel(
    ...
    final bool summaryBusy = state.isGeneratingSummary;
    final bool allBusy = busy || summaryBusy;
      ...
      children: <Widget>[
        Wrap(
          ...
          children: <Widget>[
            if (widget.showUploadButton)
              ElevatedButton.icon(
                onPressed: allBusy ? null : widget.controller.pickPdf,
                icon: const Icon(Icons.upload_file),
                label: Text(
                  state.isPickingPdf ? 'Opening...' : 'Upload PDF',
                ),
            ...
            ElevatedButton.icon(
              onPressed: (!state.hasExtractedText || state.isSpeaking || allBusy)
                  ? null
                  : widget.controller.startSpeaking,
              icon: const Icon(Icons.volume_up),
              label: const Text('Read Aloud'),
            ...
            ElevatedButton.icon(
              onPressed: (!state.hasExtractedText || allBusy || state.isSpeaking)
                  ? null
                  : () {
                widget.controller.generateSummary(
                  maxSentences: 5,
                );
              },
              icon: const Icon(Icons.summarize),
              label: Text(
                state.isGeneratingSummary ? 'Summarizing...' : 'Summary',
              ),
            ),
          ],
        ),
        ...
        if (state.summaryText.trim().isNotEmpty) ...<Widget>[
          const SizedBox(height: 12),
          Container(
            width: double.infinity,
            padding: const EdgeInsets.all(12),
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (BuildContext context, _) {
        final PdfAudioReaderState state = widget.controller.state;
        final bool busy =
            state.isPickingPdf || state.isExtractingText || state.isGeneratingSummary;
...
class LocalPdfSummarizer {
  static String summarize(
    ...
    final List<String> sentences = _splitSentences(cleanedText)
        .map((String e) => e.trim())
        .where((String e) => e.length >= minSentenceLength)
        .toList();
    ...
    final Map<String, int> wordFrequency = <String, int>{};

    for (final String sentence in sentences) {
      final List<String> words = _tokenize(sentence);
    ...
    final List<_ScoredSentence> scoredSentences = <_ScoredSentence>[];
    ...
    for (int i = 0; i < sentences.length; i++) {
      ...
      // First few sentences often contain intro/main topic, so small boost.
      if (i < 3) {
        score += 0.20;
      }
    ...
    scoredSentences.sort(
          (_ScoredSentence a, _ScoredSentence b) => b.score.compareTo(a.score),
    );

    final List<_ScoredSentence> selected = scoredSentences
        .take(maxSentences)
        .toList()
      ..sort(
            (_ScoredSentence a, _ScoredSentence b) => a.index.compareTo(b.index),
      );

    return selected.map((_ScoredSentence e) => e.sentence).join(' ');
  }
